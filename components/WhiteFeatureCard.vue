...
<template>
  <FeatureCard 
    ...
    borderClass="border-[1px] border-gray-200/100 rounded-[24px]"
    ...
    <template v-slot:default>
      ...
      <div class="bg-white/50 backdrop-blur-xl rounded-[0px] border-t-[1px] mt-5 border-gray-200/100 min-h-[52px] flex items-center justify-center cursor-pointer group-hover:underline">
        <!-- vvv Can't seem to give the inline play button image a shadow vvv -->
        <a class="text-lg dark-bg text-slate-900/90 text-center shadow-black/100"><span class="">{{ $t('feature-card.expand-button') }}</span><span class="inline-space-[8]"/><img src="~/assets/img/play.circle-dark@2x.png" alt="Play Video Icon" class="ml-[0px] inline h-[1.16rem] align-[-3.6px] filter brightness-0 drop-shadow-xl"></a>
      </div>
    </template>
