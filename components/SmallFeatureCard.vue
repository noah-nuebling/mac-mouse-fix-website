...
<template>
  <FeatureCard 
    ref="thisCard"
    class="w-fit feature-card col-span-auto group shadow-md rounded-[24px]" 
    borderClass="border-[4px] border-gray-50/25 rounded-[24px]"

    @click="$refs.thisCard.expand()">

    <template v-slot:top> 
      <h3 class="text-2xl font-bold text-white/100 m-6 shadow-black/100 w-fit whitespace-nowrap">{{ $t(titleKey) }}</h3>
    </template>
