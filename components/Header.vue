<template>
    <!-- Header -->
  <header :class="['fixed left-0 right-0 header-shadow backdrop-blur-[20px] backdrop-saturate-[1.8] z-50', darkAppearance ? 'bg-gray-900/80 text-white/[0.85]' : 'bg-gray-50/80 text-black/[0.85]']">
    <nav class="container mx-auto py-[0.4rem] text-[1rem] font-[400] flex justify-between items-center">
      ...
      <ul class="flex gap-[2rem] mx-[2rem] font-[400] my-[1.0rem] items-center h-fit tracking-[-0.01em]">
        ...
        <li><NuxtLink to="/test" class="">Überblick</NuxtLink></li> <!-- NuxtLink dynamically generates the page instead of requesting prerendered page from the server -->
        <li><NuxtLink to="/" class="">More Info</NuxtLink></li>
        <li>
          <NuxtLink to="/about" class="">GitHub <img :src="externalLinkImagePath" alt="" :class="['inline h-[0.9em] ml-[0.1em] mr-[0.1em] translate-y-[0.08em] align-baseline opacity-[0.8]', darkAppearance ? 'invert' : '']"></NuxtLink>
        </li>

        <li class="my-[-10rem]"><DownloadButton class="bg-blue-500 rounded-full text-white px-[0.85em] py-[0.3em] text-[1.0rem] translate-y-[-0.05em]"/></li>
